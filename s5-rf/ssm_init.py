"""
copy of https://github.com/lindermanlab/S5/blob/main/s5/ssm_init.py
"""
import jax
from jax import random
import jax.numpy as jnp
from jax.nn.initializers import lecun_normal, he_normal
...
def init_A(block_size: int, num_blocks: int) -> tuple[jax.Array, jax.Array, jax.Array]:
    ...
    Returns:
        Lambda (complex64): diagonal state matrix (P,2)
    ...
    Lambda = (Lambda * jnp.ones((num_blocks, block_size))).ravel()
    V = jax.scipy.linalg.block_diag(*([V] * num_blocks))
    Vinv = jax.scipy.linalg.block_diag(*([Vc] * num_blocks))

    Lambda = jnp.stack([Lambda.real, Lambda.imag], axis=-1) 
